...
export default class AppBanco extends Component { 
    ...
    hacerPlazoFijo(){
        this.setState({ plazoFijo: 'Correo: '+ 
        this.state.correo+' \nCUIT: '+ 
        this.state.cuit+' \nDias: '+ 
        this.state.slider.dias+' \nMonto: '+ 
        ((this.state.moneda==1)?'US$':'AR$') + ' '+ 
        this.state.monto+' \nIntereses: '+ 
        ((this.state.moneda==1)?'US$':'AR$') + ' '+ 
        this.intereses()+' \nAvisar por mail: '+ 
        (this.state.avisarPorMail?'Si':'No') +' \nTerminos y Condiciones: '+ 
        (this.state.aceptoTyC?'Aceptados':'Faltan aceptar')})
    }
    ...
    render() { 
        ...
        <View style={styles.container}> 
            ...
            <TextInput  style={{height: 40, borderColor: 'gray', borderWidth: 1}}
                        defaultValue={this.state.monto.toString()}
                        keyboardType='numeric' onChangeText={(val) => this.state.monto = parseFloat(val)}>
            </TextInput> 
            ...
            <Slider minimumValue={10}
                    ...
                    onValueChange={val => this.actualizarSlider(val)}>
            </Slider> 
            <Text>
                {this.state.slider.dias} dias
            </Text>

            <Text>
                Avisar por mail
            </Text> 
            <Switch value={this.state.avisarPorMail} 
                    onChange={() => this.switchCambiado()}>
            </Switch> 
